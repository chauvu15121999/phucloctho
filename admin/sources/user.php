...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			$row = $d->fetch_array();
			if($row['role'] == 3) transfer("Bạn không có quyền trên tài khoản này.<br>Mọi thắc mắc xin liên hệ quản trị website.", "index.php?com=user&act=man".$urlcu);
		}
		// xoa item
		$sql = "delete from #_user where id='".$id."'";
		if($d->query($sql))
			transfer("Dữ liệu đã được xóa", "index.php?com=user&act=man".$urlcu);
		else
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=user&act=man".$urlcu);
		...
		}else transfer("Không nhận được dữ liệu", "index.php?com=user&act=man".$urlcu);
}
