...
function save_item(){
	...
	$file_name_icon=$_FILES['file_icon']['name'];
	$file_name_h2=$_FILES['file_h2']['name'];
	...
	if($id){
		...
		if($photo1 = upload_image("file_h2", _format_duoihinh, _upload_tintuc,$file_name_h2)){
			$data['photo1'] = $photo1;
			if(_width_thumb > 0 and _height_thumb > 0)
			$data['thumb1'] = create_thumb($data['photo1'], _width_thumb, _height_thumb, _upload_tintuc,$file_name_h2,_style_thumb);
			$d->setTable('news');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_tintuc.$row['photo1']);
				delete_file(_upload_tintuc.$row['thumb1']);
			}
		}
		if($download = upload_image("file_download", _format_duoitailieu, _upload_tintuc,$file_name_download)){
			...
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_tintuc.$row['download']);
			}
		}
		if($icon = upload_image("file_icon", _format_duoihinh, _upload_tintuc,$file_name_icon)){
			$data['icon'] = $icon;
			$d->setTable('news');
			$d->setWhere('id', $id);
			$d->select();
