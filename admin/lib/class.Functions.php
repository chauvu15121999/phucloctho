<?php 
	class Functions
	{
		...
		public function price($newPrice="", $oldPrice="", $promotionPrice="")
		{
			...
			if($oldPrice > 0) $result .= '<p class="old-price px-2 py-1"><span>Giá thị trường</span> '.$this->money($oldPrice).'</p>';
			return $result;
		}
		...
		public function priceDiscount($newPrice="", $oldPrice="", $promotionPrice="")
		{
			...
			$result .= '
				<div class="priceDiscount d-flex">
					<p class="percentPrice px-2 py-1">-'.(int)$percent.'% Giảm</p>
					<p class="savingPrice px-2 py-1">Tiết kiệm: '.$saving.'</p>
				</div>
			';
			return $result;
		}
