...
<div class="widget">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
      ...
      <tr>
        <td></td>
        <td class="tb_data_small" style="padding: 0px;"><a href="#" class="tipS">STT</a></td>
				<?php if(in_array('danhmuc',$config['type'])) { ?>
        <td class="tb_data_small" style="width: 10%;"><?=get_main_danhmuc()?></td>
			  <?php } ?>
        ...
        <td class="sortCol"><div>Tên sản phẩm</div></td>
        <td width="100px" class="none"><div>Mã sản phẩm<span></span></div></td>
				<?php } ?>
				<?php if(in_array('hinhanh',$config['type'])) { ?>
        <td width="100px">Hình ảnh</td>
				<?php } ?>
				<?php if(in_array('binhluan',$config['type'])) { ?>
        <td width="130px" class="">Đánh giá</td>
        <td width="130px" class="">Hỏi đáp</td>
				<?php } ?>
		<!-- <td class="tb_data_small">Google shopping</td> -->
        ...
        <td class="tb_data_small">K.Mãi</td>
				<?php } ?>
    ...
    <tbody>
      ...
      <td align="center" class="" style="width: 10%;">
        ...
        <td class="title_name_data">
			...
			<div><b><?=$items[$i]['masp']?></b></div>
			<div style="margin-top: 5px;"><a style="color: red" href="index.php?com=video&act=man&type=video&id_pro=<?=$items[$i]['id']?>">VIDEO</a></div>
        </td>
		<td class="title_name_data none">
			...
			<?php if(in_array('hinhanh',$config['type'])) { ?>
        <td align="center">
            <a href="index.php?com=product&act=edit&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_thuonghieu=<?=$items[$i]['id_thuonghieu']?>&type=<?=$_REQUEST['type']?>&p=<?=$_REQUEST['p']?>&id=<?=$items[$i]['id']?>" class="tipS SC_bold"><img src="<?=_upload_sanpham.$items[$i]['thumb']?>" style="max-height:40px; max-width:70px;" /></a>
        </td>
			<?php } ?>
			<?php if(in_array('binhluan',$config['type'])) { ?>

         <td align="center">
            <a href="index.php?com=comment&act=edit&product_id=<?=$items[$i]['id']?>&type=<?=$_REQUEST['type']?>&id=<?=$items[$i]['id']?>&comment_type=nhanxet" class="tipS SC_bold">
             <?php
			 		$d->reset();
					$sql = "select count(id) as dem from table_comment where product_id='".$items[$i]['id']."' and type='".$_GET['type']."' and comment_type='nhanxet'";
					$d->query($sql);
		...
		<td align="center">
            <a href="index.php?com=comment&act=edit&product_id=<?=$items[$i]['id']?>&type=<?=$_REQUEST['type']?>&id=<?=$items[$i]['id']?>&comment_type=hoidap" class="tipS SC_bold">
             <?php
			 		$d->reset();
					$sql = "select count(id) as dem from table_comment where product_id='".$items[$i]['id']."' and type='".$_GET['type']."' and comment_type='hoidap' ";
					$d->query($sql);
					$dem_comment = $d->result_array();
			  ?>
            (<?=$dem_comment[0]['dem']?>)</a>
			...
			<?php } ?>
		<!--	<td align="center">
        <a data-val2="table_<?=$_GET['com']?>" rel="<?=$items[$i]['google_shop']?>" data-val3="google_shop" class="diamondToggle <?=($items[$i]['google_shop']==1)?"diamondToggleOff":""?>" data-val0="<?=$items[$i]['id']?>" ></a> 
        </td>-->
			<?php if(in_array('noibat',$config['type'])) { ?>
        ...
        <a data-val2="table_<?=$_GET['com']?>" rel="<?=$items[$i]['noibat']?>" data-val3="noibat" class="diamondToggle <?=($items[$i]['noibat']==1)?"diamondToggleOff":""?>" data-val0="<?=$items[$i]['id']?>" ></a>
			...
			<?php if(in_array('hot',$config['type'])) { ?>
        <td align="center">
        <a data-val2="table_<?=$_GET['com']?>" rel="<?=$items[$i]['hot']?>" data-val3="hot" class="diamondToggle <?=($items[$i]['hot']==1)?"diamondToggleOff":""?>" data-val0="<?=$items[$i]['id']?>" ></a>
        </td>
			<?php } ?>

			<?php if(in_array('sale',$config['type'])) { ?>
        <td align="center">
        <a data-val2="table_<?=$_GET['com']?>" rel="<?=$items[$i]['sale']?>" data-val3="sale" class="diamondToggle <?=($items[$i]['sale']==1)?"diamondToggleOff":""?>" data-val0="<?=$items[$i]['id']?>" ></a>
        </td>
			<?php } ?>
        ...
        <td class="actBtns">
            <a href="index.php?com=product&act=edit&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_thuonghieu=<?=$items[$i]['id_thuonghieu']?>&type=<?=$_REQUEST['type']?>&p=<?=$_REQUEST['p']?>&id=<?=$items[$i]['id']?>" title="" class="smallButton tipS" original-title="Sửa sản phẩm"><img src="./images/icons/dark/pencil.png" alt=""></a>
