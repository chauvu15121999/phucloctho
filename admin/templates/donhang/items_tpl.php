...
?>
<style>
	.widget td .pagination a,.widget td .pagination span{ height: 25px; padding: 4px 8px; text-decoration: none; color: #666666; font-weight: bold; background: url(images/backgrounds/pagingBg.png) repeat-x 0 0; border: 1px solid #d5d5d5; font-size: 11px; }
	.widget td .pagination span,.widget td .pagination a:hover { background: url(images/backgrounds/sideGradient.png) repeat-x; color: #fff; border-color: #424852; }
</style>
       ...
       <td>
        ...
        <td>
               <?=$items[$i]['hoten']?>
                ...
                <td align="center">
               <?=date('d/m/Y H:i A',$items[$i]['ngaytao'])?>
                </td>
      
        <td align="center">
         <?php
					$d->reset();
					$sql_chitietdonhang = "select gia,giacu,soluong,tiente from #_chitietdonhang where madonhang='".$items[$i]['madonhang']."' order by stt,id desc";
					$d->query($sql_chitietdonhang);
					$chitietdonhang = $d->result_array();
					$tongtiendonhang = 0;
					$tongtiendonhang_usd = 0;
					$soluongchiphi = 0;
				?>
                 <?php for($j=0, $count_chitietdonhang=count($chitietdonhang); $j<$count_chitietdonhang; $j++){?>
                 	<?php 
						if($chitietdonhang[$j]['giacu']>0){
							if($chitietdonhang[$j]['tiente']==1){
								$tongtiendonhang = $tongtiendonhang + ($chitietdonhang[$j]['giacu']*$chitietdonhang[$j]['soluong']);
							}
							if($chitietdonhang[$j]['tiente']==2){
								$tongtiendonhang_usd = $tongtiendonhang_usd + ($chitietdonhang[$j]['giacu']*$chitietdonhang[$j]['soluong']);
							}
							...
							if($chitietdonhang[$j]['tiente']==2){
								$tongtiendonhang_usd = $tongtiendonhang_usd + ($chitietdonhang[$j]['gia']*$chitietdonhang[$j]['soluong']);	
							}
						}
                 ...
                 <?=number_format($tongtiendonhang,0, ',', '.')?>&nbsp;vnđ + <?=number_format($tongtiendonhang_usd,0, ',', '.')?>&nbsp;usd
        </td>
       	<td align="center">
       		<?=number_format($items[$i]['phi_vanchuyen'],0, ',', '.')?>&nbsp;vnđ
       	</td>
        <td align="center">
           <?php 
        ...
        <td class="actBtns">
            ...
            <a href="print.php?id=<?=$items[$i]['id']?>" title="" class="smallButton tipS" original-title="Xuất đơn hàng"><img src="./images/icons/dark/excel.png" alt=""></a>
