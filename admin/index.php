<?php
	...
	if($_GET['com']=='comment'){
		$type_phanquyen=$_GET['comment_type'];
	}else{
		$type_phanquyen=$_GET['type'];
	}
	
	if($_SESSION['login_admin']['com']=='user'){
		$edit_noidung_email = false;
		$d->reset();
		$sql = "select com,act,type from #_com_quyen where id_quyen='".$_SESSION['login_admin']['nhom']."' limit 0,1";
		$d->query($sql);
		$arr_quyen = $d->result_array();
		if(!empty($arr_quyen)){
			foreach($arr_quyen as $k=>$v){
				if($v['type']=='noidungmail'){
					if(in_array('edit', explode(',', $v['act']))){
		...
		// var_dump($edit_noidung_email);

		$arr_edit_email = array('tieude1','tieude2','tieude3','tieude4','tieude5','tieude6','tieude7','tieude8','tieude9','tieude10','tieude11','noidung1','noidung2','noidung3','ketnoi');

		if(($_GET['com']=='about' || $_GET['com']=='slider') && ($_GET['act']=='capnhat' || $_GET['act']=='man_photo')){
			if($edit_noidung_email==true && in_array($_GET['type'],$arr_edit_email)){

			}else{
				if(phanquyen($_SESSION['login_admin']['com'],$_SESSION['login_admin']['nhom'],$_GET['com'],$_GET['act'],$type_phanquyen)){
...
<style type="text/css">
	.page-loading {
	    ...
	    width: 100%;
	    height: 100%;
	    background: rgba(255, 255, 255, 0.9) url(../images/loading2.gif) no-repeat center center;
	    z-index: 99999;
	    display: none;
	}
</style>
...
<script type="text/javascript">
	...
	$(document).on("click","a.diamondToggle1",function() {
		if($(this).attr("rel")==0){
			$(this).addClass("diamondToggleOff");
			$(this).attr("rel",1);
			$(this).next().val(1);
	...
	$(document).ready(function() {
		var w=$('.withCheck tbody tr td').first().width(); 
