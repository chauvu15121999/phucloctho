...
?>
<?php if($type == 'video-srcoll') {
	$video_home = $d->rawQuery("select link_video, id, ten$lang from #_photo where type = ? and act <> ? and hienthi > 0 order by stt, id desc",array('video','photo_static')); if(count($video_home)) { ?>
	<!-- Video Scroll -->
	<div class="form-row">
		<div class="col-sm-9">
			...
			<div class="video-right">
				<ul>
					<?php foreach($video_home as $k => $v) { ?>
			            <li class="scale-img" data-id="<?=$v['id']?>">
			            	<img class="lazy w-100" data-src="https://img.youtube.com/vi/<?=$func->getYoutube($v['link_video'])?>/mqdefault.jpg" alt="<?=$v['ten'.$lang]?>">
			            </li>
			        <?php } ?>
				</ul>
			</div>
...
<?php if($type == 'video-fotorama') {
	...
	<div id="fotorama-videos" data-width="100%" data-thumbmargin="5" data-height="360" data-fit="cover" data-thumbwidth="100" data-thumbheight="85" data-allowfullscreen="false" data-nav="thumbs">
	    <?php foreach($video_home as $k => $v) { ?>
	        <a href="https://youtube.com/watch?v=<?=$func->getYoutube($v['link_video'])?>" title="<?=$v['ten'.$lang]?>"></a>
	    <?php } ?>
	</div>
