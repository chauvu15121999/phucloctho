...
case "ghtk":
	...
	function ghtk(){
		...
		global $d,$func,$model;
		$id = $_POST['id'];
		$order = $d->rawQueryOne("select * from #_order where id = '".$id."'");
		
		$order_detail = $d->rawQuery("select * from #_order_detail where id_order = ? order by id desc",array($id));
		$data = array();
		
		$data['id'] = $order['madonhang'];
		$data['tel'] = $order['dienthoai'];
		$data['name'] = $order['hoten'];
		$data['province'] = $func->get_places("city",$order['city']);
		$data['district'] = $func->get_places("district",$order['district']);
		$data['ward'] = $func->get_places("wards",$order['wards']);
		$data['address'] = $order['diachi'];
		$data['hamlet'] = "Khác";
		$data['value'] = $order['tamtinh'];
		$data['pick_money'] = $order['tamtinh'];
		$products = array();
		foreach($order_detail as $k=>$v){
			$p = $d->rawQueryOne("select masp from #_product where id = ?",array($v['id_product']));
			$products[] = array("name"=>$v['ten'].(($v['size'])?' - '.$v['size']:'').($v['mau']?' - '.$v['mau']:''),'weight'=>'0.4','quantity'=>$v['soluong'],'product_code'=>$p['masp']);
		}
		$response = $model->sendOrder($data,$products);
		$response = json_decode($response,true);
		$data = array();
		$data['success'] = $response['success'];
		if($response['success']){
			$ship = $response['order']['fee'] + $response['order']['insurance_fee'];
			$tonggia = $order['tamtinh'] + $ship;
			$d->rawQuery("update #_order set phiship='".$ship."',tonggia='".$tonggia."',ghtk='".$response['order']['label']."',ghtk_data='".json_encode($response['order'])."' where id = '".$id."'");
			$data['msg'] = "Tạo vận đơn thành công. Mã vận đơn: ".$response['order']['label'];
			$data['code'] = $response['order']['label'];
			$data['ship'] = $func->format_money($ship);
		}else{
			$data['msg'] = "Tạo vận đơn không thành công: ".$response['message'];
		}
