<?php
	...
	function get_item()
	{
		global $d, $func, $strUrl, $curPage, $item, $gallery, $type, $com, $act, $sizes;
		...
		if(!empty($item['id_size']))
		{
			...
			$sizes = $d->rawQuery("select tenvi, id from #_product_size where id in (".$item['id_size'].") order by stt,id desc");
		}
	}
	...
	function save_item()
	{
		...
		if($id && $act!='save_copy')
		{
			...
			if($d->update('product',$data))
			{
				...
				{
					$price_size = array();
					$price_size['id_product'] = $id;
					$d->rawQuery("delete from #_product_price where id_product = ?",array($id));

					foreach($_POST['price_size']['id'] as $k => $v)
					{
						if(!empty($_POST['price_size']['id'][$k]) && !empty($_POST['price_size']['gia'][$k]))
						{
							$price_size['id_size'] = $_POST['price_size']['id'][$k];
							$price_size['gia'] = (!empty($_POST['price_size']['gia'][$k])) ? str_replace(",","",$_POST['price_size']['gia'][$k]) : 0;
							$price_size['giacu'] = (!empty($_POST['price_size']['giacu'][$k])) ? str_replace(",","",$_POST['price_size']['giacu'][$k]) : 0;
							$price_size['giakm'] = (!empty($_POST['price_size']['giakm'][$k])) ? $_POST['price_size']['giakm'][$k] : 0;
							$d->insert('product_price',$price_size);
						}
					}
				}
		...
		else
		{
			...
			if($d->insert('product',$data))
			{
				...
				{
					$price_size = array();
					$price_size['id_product'] = $id_insert;
					$d->rawQuery("delete from #_product_price where id_product = ?",array($id));
	...
	function save_list()
	{
		...
		if($id)
		{
			...
			if(isset($_FILES['file-2']))
			{
				...
				if($photo2 = $func->uploadImage("file-2", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT,$file_name))
				{
					$data['photo2'] = $photo2;
			...
			if(isset($_FILES['file-3']))
			{
				...
				if($photo3 = $func->uploadImage("file-3", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT,$file_name))
				{
					$data['photo3'] = $photo3;
					$row = $d->rawQueryOne("select id, photo3 from #_product_list where id = ? and type = ? limit 0,1",array($id,$type));
					if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_PRODUCT.$row['photo3']);
				}
			}
	...
	function delete_list()
	{
		...
		if($id)
		{
			...
			/* Lấy dữ liệu */
			$row = $d->rawQueryOne("select id, photo, photo2, photo3 from #_product_list where id = ? and type = ? limit 0,1",array($id,$type));

			if(isset($row['id']) && $row['id'] > 0)
			{
				$func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				$func->delete_file(UPLOAD_PRODUCT.$row['photo2']);
				$func->delete_file(UPLOAD_PRODUCT.$row['photo3']);
