                        ...
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
								...
								<td class="align-middle code">
									<?php 
										if(!$items[$i]['ghtk']){
											$address = $items[$i]['diachi'].' - '.$func->get_places("wards",$items[$i]['wards']).', '.$func->get_places("district",$items[$i]['district']).', '.$func->get_places("city",$items[$i]['city']);
											echo '<a href="" class="send-ghtk" data-id="'.$items[$i]['id'].'" data-address="'.$address.'">Gửi GHTK</a>';
										}else{
											echo $items[$i]['ghtk'];
										}
									?>
								...
								<td class="align-middle">
                                    <span class="text-danger font-weight-bold ship-place"><?=$func->format_money($items[$i]['phiship'])?></span>
                                </td>
                                <td class="align-middle">
                                    <span class="text-danger font-weight-bold"><?=$func->format_money($items[$i]['tamtinh'])?></span>
                                </td>
                                <td class="align-middle">
                                    <?php
...
<script>
	$().ready(function(){
		$(".send-ghtk").click(function(){
			obj = $(this);
			if(confirm("Bạn có chắc chắn gưi đơn hàng qua giao hàng tiết kiệm? Nếu sai bạn phải vào GHTK để điều chỉnh. Địa chỉ khách: "+$(this).data("address"))){
				$.ajax({
					url:"index.php?com=order&act=ghtk",
					...
					success:function(data){
						alert(data.msg);
						if(data.success){
							obj.parents('tr').find('.ship-place').html(data.ship);
							obj.parents('tr').find('.code').html(data.code);
						}
