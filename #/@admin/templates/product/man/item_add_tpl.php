<?php
	...
	function get_size($id=0)
	{
		...
		$str = '<select id="size_group" name="size_group[]" class="select multiselect-size multiselect" multiple="multiple" >';
							    ...
							    <?php if(isset($config['product'][$type]['size']) && $config['product'][$type]['size'] == true) { ?>
									...
									<div class="form-group-sizes col-12">
										<div class="group-sizes row">
							                <?php if(!empty($sizes) && !empty($item['id'])) { foreach($sizes as $v) {
							                	$price = $d->rawQueryOne("select gia, giamoi, giakm from #_product_price where id_product = ? and id_size = ? limit 0,1",array($item['id'], $v['id'])); ?>
	    ...
	    <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <div class="row">
				    ...
				    <?php } ?>
				    <?php if(isset($config['product'][$type]['giacu']) && $config['product'][$type]['giacu'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        <label class="d-block" for="giacu">Giá cũ:</label>
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price gia_moi" name="data[giacu]" id="giacu" placeholder="Giá mới" value="<?=@$item['giacu']?>">
	                        	<div class="input-group-append">
	                        		<div class="input-group-text"><strong>VNĐ</strong></div>
	                        	</div>
	                        </div>
