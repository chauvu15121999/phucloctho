<script type="text/javascript">
		...
		if($("#jFilerSortable").length)
		{
			sortable = new Sortable.create(document.getElementById('jFilerSortable'),{
				...
				onEnd: function(){
					...
					/* Update number */
					var idmuc = <?=(!empty($id)) ? $id : 0?>;
					var com = '<?=(!empty($com)) ? $com : ''?>';
					var kind = '<?=(!empty($act)) ? $act : ''?>';
	...
	function previewWatermark()
	{
		...
		$.ajax({
			type:'POST',
			url: "index.php?com=photo&act=save-watermark&type=<?=(!empty($type)) ? $type : ''?>",
			data: formData,
			cache: false,
			contentType: false,
			processData: false,
	...
	$(document).ready(function(){
		...
		<?php if(!empty($flagGallery) && !empty($gallery)) { ?>
			/* Sort filer */
			createSortFiler();
		<?php } ?>
