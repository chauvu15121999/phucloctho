<?php  
	...
	extract($model->get("params"));
	#@$id = htmlspecialchars($_GET['id']);

	#@$idl = $model->get("params.slug_list");
	...
	{
		/* Lấy sản phẩm detail */
		$row_detail = $d->rawQueryOne("select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, noidung$lang, masp, luotxem, id_brand, id_mau, id_size, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giakm, giacu, gia from #_product where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
		...
		/* Lấy giá */
		$row_detail_price = $func->getPrice($row_detail);
		...
		if($row_detail['id_size'])
		{
			...
			$prices = $d->rawQuery("select id_size, gia, giacu, giakm from #_product_price where id_product = ? order by id desc",array($row_detail['id']));
		...
		$sql = "select photo, ten$lang, tenkhongdauvi, tenkhongdauen, giacu, gia, giakm, id_mau, id_size, id from #_product where $where order by stt,id desc $limit";
		...
		$img_json_bar = (!empty($row_detail['options'])) ? json_decode($row_detail['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $row_detail['photo']))
		{
		...
		/* breadCrumbs */
		if(!empty($title_crumb)) $breadcr->setBreadCrumbs($com,$title_crumb);
		if(!empty($pro_list)) $breadcr->setBreadCrumbs($pro_list[$sluglang],$pro_list['ten'.$lang]);
		if(!empty($pro_cat)) $breadcr->setBreadCrumbs($pro_cat[$sluglang],$pro_cat['ten'.$lang]);
		if(!empty($pro_item)) $breadcr->setBreadCrumbs($pro_item[$sluglang],$pro_item['ten'.$lang]);
		if(!empty($pro_sub)) $breadcr->setBreadCrumbs($pro_sub[$sluglang],$pro_sub['ten'.$lang]);
	...
	else if($idl!='')
	{
		...
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (!empty($pro_list['options'])) ? json_decode($pro_list['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $pro_list['photo']))
		{
	...
	else if($idc!='')
	{
		...
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (!empty($pro_cat['options'])) ? json_decode($pro_cat['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $pro_cat['photo']))
		{
	...
	else if($idi!='')
	{
		...
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (!empty($pro_item['options'])) ? json_decode($pro_item['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $pro_item['photo']))
		{
	...
	else if($ids!='')
	{
		...
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (!empty($pro_sub['options'])) ? json_decode($pro_sub['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $pro_sub['photo']))
		{
	...
	else if($idb!='')
	{
		...
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (!empty($pro_brand['options'])) ? json_decode($pro_brand['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $pro_brand['photo']))
		{
