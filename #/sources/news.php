<?php  
	...
	if($id!='')
	{
		...
		if(!empty($news_list)) $breadcr->setBreadCrumbs($news_list[$sluglang],$news_list['ten'.$lang]);
		if(!empty($news_cat)) $breadcr->setBreadCrumbs($news_cat[$sluglang],$news_cat['ten'.$lang]);
		if(!empty($news_item)) $breadcr->setBreadCrumbs($news_item[$sluglang],$news_item['ten'.$lang]);
		if(!empty($news_sub)) $breadcr->setBreadCrumbs($news_sub[$sluglang],$news_sub['ten'.$lang]);
	...
	else if($idl!='')
	{
		...
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (!empty($news_list['options'])) ? json_decode($news_list['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $news_list['photo']))
		{
	...
	else if($idc!='')
	{
		...
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (!empty($news_cat['options'])) ? json_decode($news_cat['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $news_cat['photo']))
		{
	...
	else if($idi!='')
	{
		...
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (!empty($news_item['options'])) ? json_decode($news_item['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $news_item['photo']))
		{
	...
	else if($ids!='')
	{
		...
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (!empty($news_sub['options'])) ? json_decode($news_sub['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $news_sub['photo']))
		{
