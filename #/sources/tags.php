<?php  
	...
	if($id)
	{
		...
		$img_json_bar = (!empty($tags_detail['options'])) ? json_decode($tags_detail['options'],true) : null;
		if(empty($img_json_bar) || ($img_json_bar['p'] != $tags_detail['photo']))
		{
		...
		/* breadCrumbs */
		if(!empty($title_crumb)) $breadcr->setBreadCrumbs($tags_detail[$sluglang],$title_crumb);
