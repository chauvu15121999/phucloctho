<?php
	...
	$css->setCss("assets/css/user.css");
	$model->set("hide-slide",1);
	if(isset($_POST['guest'])){
		$_SESSION['guest'] = 1;
	}
	if($model->get("com")=="checkout"){
		if(!isUserLogin() && !isset($_SESSION['guest'])){
			$model->set("show-login",1);
		}
	}
	...
	if(isset($_POST['thanhtoan']))
	{
	    ...
	    $diachi2 = htmlspecialchars($_POST['diachi']);
		...
		$tamtinh = $cart->get_order_total();
		$ship = getShip($city,$district);
		$total = $tamtinh + $ship;
	    $ngaydangky = time();
	    $chitietdonhang = '';
		...
		$id_user = (isset($_SESSION[$login_member]['id'])) ? $_SESSION[$login_member]['id'] : 0;

		if(!isUserLogin()){
			$rows = $d->rawQuery("select id from #_member where email='$email'");
			if(!count($rows)){
				...
				$user['diachi'] = $diachi2;
				$user['hienthi'] = 1;
				$user['maxacnhan'] = $maxacnhan;
				$maxacnhan = $func->digitalRandom(0,3,6);
				$id_user = $d->insert('member',$user);
		...
		if($id_insert)
		{
			for($i=0;$i<$max;$i++)
			{
				...
				$q = $_SESSION['cart'][$i]['qty'];
				$proinfo = $cart->get_product_info($pid);
				$price = $cart->get_product_price($pid,$_SESSION['cart'][$i]['size']);
				if($price['gia']){
					$proinfo['gia'] = $price['gia'];
					$proinfo['giamoi'] = $price['giamoi'];
					$proinfo['giakm'] = $price['giakm'];
...
function send_active_user($username)
	{
		...
		$row = $d->rawQueryOne("select id, maxacnhan, username, password, ten, email, dienthoai, diachi from #_member where email = ? limit 0,1",array($username));
		...
		$linkkichhoat = $config_base."account/login?code=".$maxacnhan;

		/* Thông tin đăng ký */
		$thongtindangky='<td colspan="2" style="padding:3px 0px 9px 0px;border-top:0;border-left:0;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal" valign="top">';
		if($tennguoidung)
		{
