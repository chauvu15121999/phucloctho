	...
	if(!empty($_POST['submit-contact']))
	{
        ...
        if(($scoreCaptcha >= 0.5 && $actionCaptcha == 'contact') || $testCaptcha == true)
		{
			...
			if(!empty($dataContact))
			{
				...
				$data['ten'] = (!empty($dataContact['ten'])) ? htmlspecialchars($dataContact['ten']) : '';
				$data['email'] = (!empty($dataContact['email'])) ? htmlspecialchars($dataContact['email']) : '';
				$data['dienthoai'] = (!empty($dataContact['dienthoai'])) ? htmlspecialchars($dataContact['dienthoai']) : '';
				$data['diachi'] = (!empty($dataContact['diachi'])) ? htmlspecialchars($dataContact['diachi']) : '';
				$data['tieude'] = (!empty($dataContact['tieude'])) ? htmlspecialchars($dataContact['tieude']) : '';
				$data['noidung'] = (!empty($dataContact['noidung'])) ? htmlspecialchars($dataContact['noidung']) : '';
			    $data['ngaytao'] = time(); 
			    $data['stt'] = 1;
			...
			{
				$func->transfer("Dữ liệu không hợp lệ. Vui lòng thử lại sau",$config_base, false);
			}
