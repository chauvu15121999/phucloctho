<?php
	class Functions
	{
		...
		public function check_login()
		{
			...
			$token = (!empty($_SESSION[$login_admin]['token'])) ? $_SESSION[$login_admin]['token'] : '';
		...
		public function check_permission()
		{
			...
			if((isset($_SESSION[$login_admin]['username']) && $_SESSION[$login_admin]['username'] == 'admin') || !empty($config['website']['debug-developer'])) return false;
			else return true;
		}
		...
		public function get_status_newsletter($tinhtrang=0, $type='')
		{
			...
			if(!empty($config['newsletter'][$type]['tinhtrang']))
			{
		...
		public function getPrice($product=array())
		{
			...
			if(!empty($product))
			{
				$id_size = (!empty($product['id_size'])) ? explode(",", $product['id_size']) : 0;
				$id_size = (is_array($id_size)) ? $id_size[0] : 0;
				$id = (!empty($product['id'])) ? $product['id'] : 0;
				$row = $this->d->rawQueryOne("select gia, giamoi, giakm from #_product_price where id_size = ? and id_product = ? limit 0,1",array($id_size, $id));
				if(!empty($row)) $array = $row;
			}
       ...
       if($num==0){
		...
		public function format_money_api($price=0)
		{
			global $optsetting,$lang;
			$str = '';
		...
		public function checkLogin()
		{
			...
			if(!empty($_SESSION[$login_member]) || !empty($_COOKIE['login_member_id']))
			{
				$flag = true;
				$iduser = (!empty($_COOKIE['login_member_id'])) ? $_COOKIE['login_member_id'] : $_SESSION[$login_member]['id'];
				...
				if($iduser)
				{
					...
					if(!empty($row['id']))
				    {
				    	$login_session = (!empty($_COOKIE['login_member_session'])) ? $_COOKIE['login_member_session'] : $_SESSION[$login_member]['login_session'];
	    ...
	    /* Get place by ajax */
		public function get_ajax_place($table='', $title_select='Chọn danh mục',$default=0)
	    {
	    	$where = '';
	    	$params = array('0');
	    	$id_parent = 'id_'.$table;
	        ...
	        $str = '<select data-value="'.$default.'" id="'.$id_parent.'" name="data['.$id_parent.']" '.$data_level.' '.$data_table.' '.$data_child.' class="form-control select2 select-place"><option value="0">'.$title_select.'</option>';
	        foreach($rows as $v)
	        {
	            if(isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
	            else $selected = "";
