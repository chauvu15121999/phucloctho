		...
		if(mysql_query($sql))
		{
			if(is_array($_SESSION['cart']))
			{
				...
				for($i=0;$i<$max;$i++){
					...
					if($q == 0) continue;
					$sql = "INSERT INTO table_chitietdonhang (madonhang,masp,ten,size,mausac,gia,tiente,soluong,tonggia,ngaytao,photo) VALUES ('$madonhang','$pmasp','$pname','$size','$mausac','$pgia','$tiente','$q','$ptonggia','$ngaydangky','$pphoto')";
					...
					*=====================================*/
					...
					$mail->CharSet = "utf-8";		
						...
						$body .= '
							...
							<tr>
								<th>Số tiền :</th><td>'.number_format(get_order_total(),0, ',', '.').' VNĐ'.' + '.number_format(get_order_total_usd(),0, ',', '.').' USD'.'</td>
							</tr>
							<tr>
						...
						if(is_array($_SESSION['cart']))
						{
							...
							for($i=0;$i<$max;$i++){
								...
								$body.='<td width="17%" align="center">'.number_format(get_price($pid),0, ',', '.').'&nbsp;<sup>'.$currency.'</sup></td>';				
								$body.='<td width="8%" align="center">'.$q.'</td>';                 
								$body.='<td width="10%" align="center" class="gh_an">'.number_format(get_price($pid)*$q,0, ',', '.') .'&nbsp;<sup>'.$currency.'</sup></td>
								</tr>';
							}
							$body.='<tr bgcolor="#535353" height="30px" style="font-weight:bold;color:#FFF"><td colspan="7">Tổng giá: '.number_format(get_order_total(),0, ',', '.').'&nbsp;<sup>đ</sup> + '.number_format(get_order_total_usd(),0, ',', '.').'&nbsp;<sup>usd</sup></td></tr>';
						}
						else{
