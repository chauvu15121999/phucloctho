<?php 
	...
	if(!empty($_POST) && isset($_POST['recaptcha_response_dknt'])){

	    $recaptchaName = $_POST['recaptcha_response_dknt'];
	    $req = array(
            'secret' => $secret_key,
		...
		curl_close($rep);
	    $rep = json_decode($rep);
	    if($rep->score >= 0.5 && $rep->action=='dknt2') {

	
	
			$ten = (string)magic_quote(trim(strip_tags($_POST['index_ten_dknt'])));
			$dienthoai = (string)magic_quote(trim(strip_tags($_POST['index_dienthoai_dknt'])));
			$email = (string)magic_quote(trim(strip_tags($_POST['index_email_dknt'])));
